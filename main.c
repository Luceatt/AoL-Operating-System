...
volatile sig_atomic_t quantum_expired = 0;  // Flag untuk tandain bahwa time quantum sudah habis atau belum.
int running_time = TIME_QUANTUM;            // Set running time awal = time quantum yang digunakan.
...
typedef struct {
    ...
    int in_queue;           // Flag untuk tandain apakah process sudah di ready queue atau belum.
} PCB;

// Simple queue untuk simpan process-process yang sudah berada di READY state.
typedef struct {
...
// Function untuk initialize queue.
...
// Function untuk cek apakah queue sedang kosong atau tidak.
int is_empty(ReadyQueue *q) {
    return q->count == 0;
}

// Function untuk cek apakah queue sedang full atau tidak.
int is_full(ReadyQueue *q) {
    return q->count == MAX_PROCESS;
}

// Function untuk memasukan process kedalam ready queue.
...
// Function untuk mengeluarkan atau mengambil process dari ready queue.
...
ReadyQueue ready_queue;  // Declare ready queuenya jadi global variable.

// Function mensimulasikan kerja suatu process dengan print message.
void child_process_work(int id) {
    while (1) {     // Process berjalan terus sampai distop oleh scheduler.
        printf("[Process P%d] Executing...\n", id);
        sleep(1);   // Simulasi 1 detik eksekusi process.
    }
}
...
// Function untuk membuat satu PCB untuk suatu process.
PCB create_pcb(int id, int arrival_time, int burst_time) {
    ...
    pcb.in_queue = 0; // Set process tidak berada di ready queue.
...
void timer_handler(int signum) {
    (void)signum;           // Untuk menghindari warning dari compiler karena unused parameter.

    quantum_expired = 1;    // Tandai flagnya bahwa time quantum telah habis.
}

// Function untuk mengaktifkan timer quantum.
void setup_timer() {
    struct itimerval timer; // Struct untuk mengatur timer OS.

    // Atur kapan SIGALRM-nya dikirim.
    timer.it_value.tv_sec = running_time;       // Atur berdasarkan running time.
    timer.it_value.tv_usec = 0;                 // Kirim SIGALRM pertama.

    // Atur interval SIGALRM berikutnya.
    timer.it_interval.tv_sec = running_time;    // Kirim berulang tiap berapa detik running time.
    timer.it_interval.tv_usec = 0;              // Kirim SIGALRM-nya.

    // Aktifkan timernya, lalu OS akan mulai menghitung timer dan SIGALRM akan dikirimkan secara otomatis.
    setitimer(ITIMER_REAL, &timer, NULL);       
}

// Function untuk stop timer saat semua proses dalam state FINISHED.
void stop_timer() {
    struct itimerval timer; // Struct untuk mengatur timer OS.

    // Set semua nilai timer ke 0, artinya timer berhenti. Tidak ada alarm lagi yang akan dikirim.
...
void setup_signal_handler() {
    struct sigaction sa;    // Declare struct untuk daftarin signal handler.

    sa.sa_handler = timer_handler;  // Kalau SIGALRM datang, jalanin function timer_handler().
    sigemptyset(&sa.sa_mask);       // Set default, jangan block signal apapun saat handler berjalan.
    sa.sa_flags = SA_RESTART;
    // Kalau system call terganggu oleh SIGALRM, continue program secara otomatis setelah handler selesai.
    // Jadi, program bisa lanjut ke baris berikutnya tanpa return error karena system callnya berhenti.

    // Daftarin handler yang sudah disetting.
...
void check_new_arrivals(PCB pcb[], int n) {
    for (int i = 0; i < n; i++) {   // Loop sampai max process.
        ...
        if (pcb[i].arrival_time <= current_time && pcb[i].state == READY && pcb[i].in_queue == 0) {
            ...
            pcb[i].in_queue = 1;        // Tandain flagnya = 1, artinya process sudah masuk ke queue.
        }
    }
}
